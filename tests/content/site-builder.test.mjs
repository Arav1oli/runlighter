...
test('site build emits valid feed, sitemap and excludes drafts',async()=>{
  ...
  const [feed,sitemap,listing,indexNowKey]=await Promise.all([readFile(fromRoot('feed.xml'),'utf8'),readFile(fromRoot('sitemap.xml'),'utf8'),readFile(fromRoot('blog','index.html'),'utf8'),readFile(fromRoot(`${config.indexNowKey}.txt`),'utf8')]);
