...
async function updateHomepageArticles(articles){
  ...
  const prioritySlugs=[
    'accounting-firm-client-onboarding-workflow-australia',
    'how-accounting-firms-stop-chasing-client-documents',
    'accounting-firm-workflow-bottlenecks-and-capacity'
  ];
    ...
    '@graph':[
      {
        ...
        name:'Run Lighter',
        url:`${siteUrl}/`,
        logo:`${siteUrl}/assets/brand/logo/runlighter-logo-primary.png`,
...
function blogJsonLd(articles,siteUrl){
  return JSON.stringify({
    '@context':'https://schema.org',
    '@graph':[
      {'@type':'Blog','@id':`${siteUrl}/blog/#blog`,url:`${siteUrl}/blog/`,name:'Run Lighter Blog',description:'Practical guides to reducing repeated work while keeping judgement and accountability human.',publisher:{'@id':`${siteUrl}/#organisation`},inLanguage:'en-AU'},
      {'@type':'ItemList','@id':`${siteUrl}/blog/#articles`,name:'Run Lighter articles',numberOfItems:articles.length,itemListElement:articles.map((article,index)=>({'@type':'ListItem',position:index+1,url:article.canonical_url,name:article.title}))}
    ]
  });
}
...
export async function buildSite(config){
  ...
  const staticUrls=['/','/book/','/privacy/','/spine.html','/automated-marketing.html','/lead-generation.html','/security.html','/accounting-firm-workflow-automation-sydney/','/accounting-workflow-check/','/blog/'];
  const sitemap=`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${staticUrls.map(url=>`<url><loc>${config.siteUrl}${url}</loc></url>`).join('')}${articles.map(article=>`<url><loc>${article.canonical_url}</loc><lastmod>${article.updated||article.date}</lastmod></url>`).join('')}</urlset>`;
