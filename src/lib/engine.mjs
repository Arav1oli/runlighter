...
import { validateContentFile, assertValid, assertFreshCreativeForPublication } from './validation.mjs';
...
export async function markWebsitePublished(config,date,{simulate=false}={}) {
  ...
  const content = await readJson(contentPath(entry.article_slug));
  const runDate = zonedParts(new Date(),config.timezone).date;
  assertFreshCreativeForPublication(content,runDate,config.timezone);
